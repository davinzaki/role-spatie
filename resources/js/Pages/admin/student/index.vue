...
const heads = ["Name", "Class", "Created at", ""];
const isLoading = ref(true);
const isRoleTeacher = ref(false);
const userRole = computed(() => usePage().props.value.teacher_role[0]);
...
onMounted(() => {
    // console.log(props.additional.role);
    console.log(userRole.value);
    // isRoleTeacher.value = props.additional.role === "teacher";
    isRoleTeacher.value = userRole.value === "teacher";
    console.log(isRoleTeacher.value);
    getData(1);
});

const getData = debounce(async (page) => {
    axios
        .get(route("student.getdata"), {
            params: {
                page: page,
...
const deleteHandle = async () => {
    axios
        .delete(route("student.delete", { id: itemSelected.value.id }))
        .then((res) => {
            notify(
                {
                    type: "success",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between items-center">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Student</h1>
    </div>
    <div
