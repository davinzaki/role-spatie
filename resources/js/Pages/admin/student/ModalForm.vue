...
const formError = ref({});
const form = ref({});
const levelSelectLoading = ref(true);
const levelSelectHandle = ref();
const levelOptions = ref([]);
const categorySelectHandle = ref();
const levelPlaceholder = ref("Select Level Category First");
...
const update = async () => {
    ...
    axios
        .post(route("student.update", { id: props.data.id }), form.value)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
const create = async () => {
    isLoading.value = true;
    axios
        .post(route("student.create"), form.value)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
<template>
    <VDialog
        ...
        :title="updateAction ? 'Update Student' : 'Create Student'"
        @opened="openForm"
        @closed="closeForm"
        size="xl"
    >
        ...
        <template v-slot:content>
            <div class="grid grid-cols-1 gap-3">
                ...
                />
                <VInput
                    placeholder="Insert Student Class"
                    label="Class"
                    :required="true"
                    v-model="form.class"
